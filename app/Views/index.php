...
<header>
    ...
    <div class="heroe">
        ...
        <h1>Welcome to CodeIgniter <?= CodeIgniter\CodeIgniter::CI_VERSION ?> Media Library Demo</h1>

        <h2>The small library that manages your media files seamlessly.</h2>
...
<section>
    ...
    <p>The page you are looking at is used for demo</p>

    <h1>Upload</h1>

    <?php if (session()->has('upload_success')) : ?>
        <p style="color: green;"><?= session('upload_success') ?></p>
    <?php endif; ?>

    <form action="<?= url_to('MediaTestController::create') ?>" method="post" enctype="multipart/form-data">
        <input type="text" name="name" id="name" placeholder="enter your file name" required>
    ...
    <form action="<?= url_to('MediaTestController::show', 'user_document') ?>" method="get" enctype="multipart/form-data">
        <select name="document_id" id="document_id" style="width: 400px;" required>
            <option value="" selected disabled>Select Document Name</option>
    ...
    <?php if (isset($media)) : ?>
        <h1>Media Data of <?= $selected_document->name ?></h1>
...
<?= print_r($media) ?>
    ...
    <h1>Delete Data with File</h1>

    <?php if (session()->has('delete_success')) : ?>
        <p style="color: green;"><?= session('delete_success') ?></p>
    <?php endif; ?>

    <form action="<?= url_to('MediaTestController::delete', 'user_document') ?>" method="post">
        <select name="document_id" id="document_id" style="width: 400px;" required>
            <option value="" selected disabled>Select Document Name</option>
            <?php foreach ($documents as $document) : ?>
                <option value="<?= $document->id ?>"><?= $document->name ?></option>
